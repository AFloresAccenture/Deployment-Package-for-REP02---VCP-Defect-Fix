  ...
  */
  ...
  -- 29-Apr-2015: End of line for defect #170 and INC960237.  
    ...
    WHEN 'ON_HAND_DATA' THEN
      ...
      v_rec.where_clause     := 'process_flag = 3 AND order_type = 18 AND abs(request_id) >= '||v_main_request_id||' AND abs(request_id) <= '||v_request_id; --8/12/2015 AFlores
    ...
    WHEN 'IN_TRANSIT' THEN
      ...
      v_rec.table_name       := 'msc_st_supplies';
      v_rec.table_type       := 'STAGING';
      v_rec.where_clause     := 'process_flag = 3 AND order_type in (11,12) AND abs(request_id) >= '||v_main_request_id||' AND abs(request_id) <= '||v_request_id; --8/12/2015 AFlores
      v_rec.group_by_clause  := 'error_text';
      v_query(1)             := v_rec;
    WHEN 'WORK_ORDERS' THEN
      ...
      v_rec.table_name       := 'msc_st_supplies';
      v_rec.table_type       := 'STAGING';
      v_rec.where_clause     := 'process_flag = 3 AND order_type = 3 AND abs(request_id) >= '||v_main_request_id||' AND abs(request_id) <= '||v_request_id; --8/12/2015 AFlores
      v_rec.group_by_clause  := 'error_text';
      v_query(1)             := v_rec;
    WHEN 'WIP' THEN
      v_rec.column_name      := 'error_text, count(*)';
      ...
      v_rec.table_type       := 'STAGING';
      v_rec.where_clause     := 'process_flag = 3 AND abs(request_id) >= '||v_main_request_id||' AND abs(request_id) <= '||v_request_id; --8/12/2015 AFlores
      v_rec.group_by_clause  := 'error_text';
      v_query(1)             := v_rec;
